 ...
 */
class AnthropicClient implements ApiClientInterface {

    private string $apiKey;
    private string $model;
    private int $maxRetries = 3;

    public function __construct( string $encryptedKey, string $model = 'claude-haiku-4-5-20251001' ) {
    ...
    public function generate( array $payload ): array {
        ...
        throw new \RuntimeException( 'Anthropic: max retries exceeded' );
    }
    ...
    private function callApi( array $payload ) {
        ...
        if ( ! is_array( $data ) || empty( $data['content'][0]['text'] ) ) {
            return new \WP_Error( 'invalid_response', 'Invalid Anthropic response' );
        }
